...
int main(void)
{
    ...
    {
        // Lock the operation of APPLaunch and let this device occupy it.
        ...
        fl.l_type = F_WRLCK; // 写锁，也叫排他锁
        fl.l_whence = SEEK_SET;
        fl.l_start = 0;
        fl.l_len = 0; // 0 表示锁整个文件
        fcntl(fd, F_SETLK, &fl);
        ftruncate(fd, 0);
        dprintf(fd, "%d\n", getpid());
        printf("process %d acquired lock: %s\n", getpid(), lock_file);
        printf("press Ctrl+C to exit and release lock\n");
    }
